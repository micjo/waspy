...
async def run_pre_channeling(sub_folder, recipe: rbs.RbsRqmRecipe, detectors: List[rbs.CaenDetectorModel]):
    ...
    for angle in angle_values:
        ...
        for detector in active_detectors:
            ...
            await store.store_histogram(sub_folder, file_stem, detector.identifier, measuring_time_msec, recipe.title, data)
    ...
    plot.plot_energy_yields_and_clear(sub_folder, recipe.file_stem, angle_values, energy_yields, smooth_angles, smooth_yields, angle_to_vary)
...
async def run_random(sub_folder, recipe: rbs.RbsRqmRecipe, detectors: List[rbs.CaenDetectorModel]):
    ...
    for detector in active_detectors:
        data = await control.get_packed_histogram(detector)
        await store.store_histogram(sub_folder, recipe.file_stem, detector.identifier, measuring_time_msec, recipe.title, data)
        plot.append_histogram_plot(detector, data)
